				...
				try {
					...
					while(TASK_STATE) {
						try {		
							...
							if(text.substring(0, 2).equals("UU")) {
								int speed = Integer.parseInt(text.substring(2, text.length()));
						        mHandler.obtainMessage(MESSAGE_UP, speed - 50).sendToTarget();
							} else if(text.substring(0, 2).equals("UR")) {
								int speed = Integer.parseInt(text.substring(2, text.length()));
						        mHandler.obtainMessage(MESSAGE_UPRIGHT, speed - 50).sendToTarget();
							} else if(text.substring(0, 2).equals("UL")) {
								int speed = Integer.parseInt(text.substring(2, text.length()));
						        mHandler.obtainMessage(MESSAGE_UPLEFT, speed - 50).sendToTarget();
							} else if(text.substring(0, 2).equals("DD")) {
								int speed = Integer.parseInt(text.substring(2, text.length()));
						        mHandler.obtainMessage(MESSAGE_DOWN, speed - 50).sendToTarget();
							} else if(text.substring(0, 2).equals("DR")) {
								int speed = Integer.parseInt(text.substring(2, text.length()));
						        mHandler.obtainMessage(MESSAGE_DOWNRIGHT, speed - 50).sendToTarget();
							} else if(text.substring(0, 2).equals("DL")) {
								int speed = Integer.parseInt(text.substring(2, text.length()));
						        mHandler.obtainMessage(MESSAGE_DOWNLEFT, speed - 50).sendToTarget();
							} else if(text.substring(0, 2).equals("RR")) {
								int speed = Integer.parseInt(text.substring(2, text.length()));
						        mHandler.obtainMessage(MESSAGE_RIGHT, speed - 50).sendToTarget();
							} else if(text.substring(0, 2).equals("LL")) {
								int speed = Integer.parseInt(text.substring(2, text.length()));
						        mHandler.obtainMessage(MESSAGE_LEFT, speed - 50).sendToTarget();
